...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	/**/
	public var quants:Array<Float> = [4, // quarter
		2, // half
		4 / 3, 1, 4 / 8]; // eight
	...
	override function create()
	{
		...
		curDiff = PlayState.storyDifficulty;
		...
		leftIcon = new HealthIcon('bf', true);
	...
	function addSongUI():Void
	{
		...
		diffDropDown = new FlxUIDropDownMenuCustom(player1DropDown.x + 140, player3DropDown.y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(CoolUtil.difficulties, true),
			function(diff:String)
			{
				curDiff = Std.parseInt(diff);
	...
	function updateHeads():Void
	{
		...
		{
			leftIcon.changeIcon(healthIconP1, true);
			rightIcon.changeIcon(healthIconP2, true);
			if (_song.notes[curSection].gfSection)
				leftIcon.changeIcon('gf', true);
		}
		else
		{
			leftIcon.changeIcon(healthIconP2, true);
			rightIcon.changeIcon(healthIconP1, true);
			if (_song.notes[curSection].gfSection)
				leftIcon.changeIcon('gf', true);
		}
	}
	...
	function loadJson(song:String):Void
	{
		...
		// var poop:String = Highscore.formatSong(song.toLowerCase(), curDiff);
		// if (sys.FileSystem.exists(Paths.json("data/" + song.toLowerCase() + "/" + poop))
		// 	|| sys.FileSystem.exists(Paths.modsJson("data/" + song.toLowerCase() + "/" + poop))
		// 	|| sys.FileSystem.exists(Paths.json("data/" + song.toLowerCase() + "/" + song.toLowerCase()))
		// 	|| sys.FileSystem.exists(Paths.modsJson("data/" + song.toLowerCase() + "/" + song.toLowerCase())))
		// {
		// can't get invalid song check to work, so... ahhh goofy - ran
		if (CoolUtil.difficulties[curDiff] != CoolUtil.defaultDifficulty && CoolUtil.difficulties[curDiff] != CoolUtil.defaultDifficulty.toLowerCase())
		{
			PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + CoolUtil.difficulties[curDiff].toLowerCase(), song.toLowerCase());
		}
		else
		{
		...
		// else
		// {
		// 	openSubState(new Prompt('This song does not exist!\nThis also appears when the difficulty doesn\'t exist and when no normal difficulty exists.', 0, function(){}, null, ignoreWarnings));
		// }
	}
	...
	private function saveLevel()
	{
		...
		{
			var difficulty:String = CoolUtil.difficulties[curDiff];
			if (difficulty == CoolUtil.defaultDifficulty)
				difficulty = '';
			else
			...
			_file.save(data.trim(), Paths.formatToSongPath(_song.song) + difficulty + ".json");
		}
	}
