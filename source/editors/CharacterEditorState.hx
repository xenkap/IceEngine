...
class CharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.pressed && FlxG.mouse.justMoved && clickHold)
		{
			...
			if (startMousePos.x - char.x + char.animationsArray[curAnim].offsets[0] >= 0
				&& startMousePos.x - char.x <= char.width
				&& startMousePos.y - char.y + char.animationsArray[curAnim].offsets[1] >= 0
				&& startMousePos.y - char.y <= char.height)
			{
